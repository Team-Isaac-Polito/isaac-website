      ...
      <div className="mx-10">
        <SegmentedControl
          ...
          segments={areas.map((areaItem) => ({
            ...
            ref: React.createRef(),
            className: "w-full tablet:w-[170px] h-12 tablet:h-10 laptop:w-[230px] laptop:h-12 notebook:w-[280px] desktop:w-[400px] notebook:h-14 m-auto",
          }))}
        />
      </div>
      {areas.map((areaItem) => {
        if (areaItem.value === area) {
          return (<Area
            ...
            members={areaItem.members.map((member) => ({
              children: (
                <img src={member.image || placeholder} alt={member.name} className="py-11" />
